...
import com.example.provincialcitystreet.adapter.AreaAdapter;
import com.example.provincialcitystreet.adapter.CityAdapter;
import com.example.provincialcitystreet.adapter.ProvinceAdapter;
import com.example.provincialcitystreet.adapter.VillageAdapter;
...
*/
        ...
        villageAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                strings.set(3, villageBeans.get(position).getName());
                tabLayout.setupWithViewPager(viewPager);
                viewPagerAdapter.notifyDataSetChanged();
    ...
    public void setSelect(int... value) {
        ...
        if (value.length == 4) {
            ...
            strings.add(addressBeans.get(value[0]).getChildren().get(value[1]).getName());
            strings.add(addressBeans.get(value[0]).getChildren().get(value[1]).getChildren().get(value[2]).getName());
            strings.add(addressBeans.get(value[0]).getChildren().get(value[1]).getChildren().get(value[2]).getChildren().get(value[3]).getName());
            tabLayout.setupWithViewPager(viewPager);
            viewPagerAdapter.notifyDataSetChanged();
